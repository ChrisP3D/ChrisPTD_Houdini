...
class State(object):
    ...
    def onMouseEvent(self, kwargs):
        ...
        if hit != -1:
            ...
            #  and if ui_event.device().isLeftButton():
            if ui_event.device().isLeftButton() and reason == hou.uiEventReason.Picked:
    ...
    def onMouseWheelEvent(self, kwargs):
        ...
        if device.isCtrlKey():
            ...
            #set variant
            new_value = self.node.parm(f"variant_id{inc}").evalAsInt() + math.floor(scroll)
    ...
    def getVariantLimits(self):
        #Get the variant limits from the min/max detail variant attribute
    ...
    def testVariantLimits(self):
        """ Test the variant limits to alert user if the variant is out of bounds"""
        inc = self.getInc()   
